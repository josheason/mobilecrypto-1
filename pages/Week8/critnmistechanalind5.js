...
export default class critnmistechanalind5 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w8_5: true})
        		})
        		batch.commit().then(() => {
	...
	return(
				...
				{'Criteria and Misconceptions of Technical Analysis and Indicators'}
				</Text>
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.shortvlonganal4()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about criteria and misconceptions of technical analysis and indicators. `}
			</Text><></>
			<Text style={styles.para}>
			{`There is a reason why majority of the course is fundamental analysis and why I call it “The Fundamental Secrets”  `}
			</Text><></>
			<Text style={styles.italPara}>
			{`Here are some common misconceptions:  `}
			</Text><></>
			<Text style={styles.bolPara}>
			{`“Technical analysis moves markets” `}
			</Text><></>
			<Text style={styles.para}>
			{`Biggest misunderstanding in cryptomarkets. Technical analysis does not singlehandedly move the markets.  `}
			</Text><></>
			<Text style={styles.para}>
			{`If this was true, then there would be a lot more random movement within the markets, as people would randomly depend on one or more indicators to place their trades. `}
			</Text><></>
			<Text style={styles.para}>
			{`When individuals do technical analysis and then trade accordingly is what will move the markets.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.bolPara}>
			{`“Technical analysis can predict everything” `}
			</Text><></>
			<Text style={styles.para}>
			{`There are movements that technical analysis cannot predict.  `}
			</Text><></>
			<Text style={styles.para}>
			{`For instance, the crash in February 2020, where fundamental analysis of the current world environment would have shown otherwise.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.bolPara}>
			{`Two criteria for technical indicators ( to determine if it is pivotal at the time) `}
			</Text><></>
			<Text style={styles.para}>
			{` 1. Does the technical indicator have a significant amount of interest from traders? `}
			</Text><></>
			<Text style={styles.para}>
			{` 2. Does this technical indicator have a history of making accurate predictions?  `}
			</Text><></>
			<Text style={styles.para}>
			{`If they do not meet the criteria, avoid them, in most cases. (MACD and RSI are good models that fit the criteria at all times) `}
			</Text><></>
			<Text style={styles.para}>
			<Text style={styles.ital}>{`A common myth:`}</Text><></>
			<Text style={styles.bol}>{`“You can have an absolute prediction about what the market will do using only technical analysis”`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			{`There is no such thing as absolutes in cryptocurrency- anything can happen. `}
			</Text><></>
			<Text style={styles.para}>
			{`Cryptocurrency trading is highly based on statistics and probabilities, where none of it is 100%.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Predict the market movement with percentages and base your trades on them. `}
			</Text><></>
			<Text style={styles.para}>
			{`Despite the precautions, your probability of being wrong is higher than being right.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.italPara}>
			{`Alongside technical analysis, you must use fundamental analysis; they go hand in hand, not one is entirely omnipotent.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
