...
export default class moving10 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w8_10: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%208%2FMovingAverages.mp4?alt=media&token=eb26c3ff-21a4-4c66-b4df-e0c8604f290d`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about simple, regular and exponential moving averages. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`It is a technical analysis tool that smooths out price action by filtering out “noise” from random short-term price fluctuations  `}
			</Text><></>
			<Text style={styles.italPara}>
			{`What does it signify? `}
			</Text><></>
			<Text style={styles.para}>
			{`It indicated a trend since it is based on past prices. `}
			</Text><></>
			<Text style={styles.italPara}>
			{`What is it used for? `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`To indicate trends reversed based on crosses.  `}
			</Text><></>
			<Text style={styles.para}>
			...
			<Text style={styles.para}>
			{`Now turn your attention to the screen (0:59), to follow along with reading moving averages on a chart `}
			</Text><></>
