...
export default class masspsych2 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w8_2: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%208%2FMassPsychology.mp4?alt=media&token=fb26f278-de5c-4548-a9c4-cd9ffcbf0a66`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.tradeviewtut1()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about mass psychology; dissecting the reasons why people make certain judgment calls. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.bolPara}>
			{`What do you need to know about markets and mass psychology? `}
			</Text><></>
			<Text style={styles.para}>
			{`The struggle: The incessant struggle between the bulls and the bears is what drives market rallies and precipitates market declines `}
			</Text><></>
			<Text style={styles.para}>
			{`Trader’s Roles: Market movements are the manifestation of the action of traders `}
			</Text><></>
			<Text style={styles.para}>
			{`TA Market movement myth: Traders move the market; the market does NOT move because of technical analysis.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Reasons why traders might place a trade: Technical analysis; Fundamental news; Mass Psychology.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.bolPara}>
			{`What forces are responsible for moving the markets? `}
			</Text><></>
			<Text style={styles.para}>
			{`Emotional forces: Optimism, pessimism, panic, etc.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Crowd nature: The way in which a collection of usually rational individuals can be overwhelmed by emotion. Once emotion appears, others will follow suit in a universal manner.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Choices: The penultimate factor that makes or breaks a trader's sell or buy.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.bolPara}>
			{`Concepts to understand mass psychology: `}
			</Text><></>
			<Text style={styles.para}>
			{`Contrarianism: The idea that an individual gets what is opposite of the normal viewpoint `}
			</Text><></>
			<Text style={styles.para}>
			{`FOMO: “Fear of Missing Out.” The idea that when markets show uptrend confidence, people start buying the asset, regardless of future outlooks.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Institutional Investors: Companies that invest money on behalf of other people.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Mainstream Media: Normal people typically are more confident with markets when they see mainstream media adopts this asset.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Smart Money: Capital placed in market by institutional investors, market havens, central banks, funds, and other financial professionals.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Bubble: Economic cycle characterized by fast increase of asset prices followed by a contraction (crash). `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`Stealth Phase: Period when bubble has not yet formed and “smart money” realizes potential for fast increase in market value. `}
			</Text><></>
			<Text style={styles.para}>
			{`Awareness Phase: Period when prices keep increasing and awareness is starting to spread about the market. A sell off occurs for smart money to keep initial profits.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Mania Phase: Period when part time or retail investors start to make money speculating than their primary occupation  `}
			</Text><></>
			<Text style={styles.para}>
			{`Blow off Phase: Period when market goes down to all time highs and quickly crashes due to unsustainable prices.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{` - Most people are wrong when they make trades. `}
			</Text><></>
			<Text style={styles.para}>
			{` - You need to look at things from multiple angles in order to profit from cryptocurrency. `}
			</Text><></>
			<Text style={styles.para}>
			{` - You can determine mass psychology by determining the trends. `}
			</Text><></>
			<Text style={styles.para}>
			{` - Markets tend to have duality. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
