...
export default class macdrsi11 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w8_11: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%208%2FMACDRSI.mp4?alt=media&token=fbfb4766-6449-4f23-b255-e537c228cf1d`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about major oscillators: MACD and RSI.`}
			</Text><></>
			<Text style={styles.para}>
			{`I recommend watching this video in order to understand the material.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`It is a trend following momentum indicator that shows the relationship between two moving averages of a security price. `}
			</Text><></>
			<Text style={styles.para}>
			{`It is a pivotal oscillating indicator that helps confirm a trend reversal (not predict)  `}
			</Text><></>
			<Text style={styles.para}>
			{`The components are MACD line, signal line, and bars. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`It is a tool in technical analysis that is used to indicate the magnitude of recent price changes to evaluate overbought or oversold conditions in price of an asset.  `}
			</Text><></>
			<Text style={styles.para}>
			{`It is a very pivotal indicator in which it indicates oversold and overbought territories.  `}
			</Text><></>
			<Text style={styles.para}>
			{`The components of RSI are a range of 0 to 100 and it is calculated and displayed as lines. `}
			</Text><></>
			<Text style={styles.para}>
			{`Follow along with the video in order to learn how to read the charts.  `}
			</Text><></>
