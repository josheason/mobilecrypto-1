...
export default class bollinger12 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w8_12: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%208%2FBollingerBandsandFibonacciRetracement.mp4?alt=media&token=fa0f12a5-b191-4715-9ad6-466b7349c7eb`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.macdrsi11()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.logregmarketcap1()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about bollinger bands and fibonacci retracement.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`This video will be tutorial style; turn your attention to the screen to follow along (0:19) `}
			</Text><></>
