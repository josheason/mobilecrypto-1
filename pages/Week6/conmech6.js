...
export default class conmech6 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w6_6: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%206%2FConsensusMechanism.mp4?alt=media&token=f969c914-2ad8-4493-a6e2-d3b8d0239b19`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.decent5()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.valueprop7()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about the consensus of a blockchain. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`What is a consensus mechanism? It is a fault-tolerant mechanism that is used in computers and  blockchain to achieve the necessary agreement on a single data value or state of the network among distributed process or multi-agree systems, such as cryptocurrencies.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Basically, nodes are competing through hashing; the first person to find the number gets access to mine the block. When they do that, they are able to add a block to the chain. Now, everyone has to come to a consensus that the block is real through proof of work.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Proof of work has been considered a very successful consensus mechanism because it improves security.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`There are a variety of consensus mechanisms; I encourage you to research them all and learn which one your coin uses.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Regarding new technology, make sure people are already talking about it before you invest in it. `}
			</Text><></>
			<Text style={styles.para}>
			{`Ultimately, just understand what it is and make sure it is credible. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
