...
export default class decent5 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w6_5: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%206%2FDecentalizationUSE.mp4?alt=media&token=a9e67a9f-7be8-4385-803a-27af09fcf84f`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about decentralization.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Here are some characteristics of decentralization:  `}
			</Text><></>
			<Text style={styles.para}>
			...
			<Text style={styles.para}>
			{`   \u2022 Network must be free of permission  `}
			</Text><></>
			<Text style={styles.para}>
			...
			<Text style={styles.para}>
			{`Lastly, I want to touch on Google services.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Google came out with a search algorithm that is extremely popular.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Then, they provide tons of free apps such as: Gmail, Docs, Drive, etc. `}
			</Text><></>
			<Text style={styles.para}>
			{`The reason why it is free is because they take your data and sell it to the highest bidder.  `}
			</Text><></>
			<Text style={styles.para}>
			{`This is a huge centralization risk; Google has too much power.  `}
			</Text><></>
			<Text style={styles.para}>
			{`We will get deeper into this, but I want you to have this background knowledge.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Reminder: Make sure you're taking notes; I do not want you to start slacking at this point in the course. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
