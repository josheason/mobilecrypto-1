...
export default class defi12 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w2_12: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: 'https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%202%2Fdefi.mp4?alt=media&token=93affbd9-14a8-4f48-9046-6f1e90d3d6f6'}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.dapps11()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.week3intro1()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will do a quick introduction video to DEFI.`}
			</Text><></>
			<Text style={styles.para}>
			{`Let’s get started.`}
			</Text><></>
			...
			<Text style={styles.para}>
			{`Notice a trend here: we are taking real world use cases that are successful and bringing them into blockchain technology. `}
			</Text><></>
			<Text style={styles.para}>
			{`DEFI is driving a lot of adoptions, this is because the majority of individuals get into cryptocurrency to make money; people favor this over the games and gambling found on DAPPS, that is why DEFI is doing so well.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`On DEFI you have cryptocurrency with a contract, with this you can: `}
			</Text><></>
			<Text style={styles.para}>
			{`\u2022 Pull Money`}
			</Text><></>
			...
			<Text style={styles.para}>
			{`Essentially, I am not telling you to go all in on DEFI; however, I want you to be aware of it. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
