...
export default class dapps11 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w2_11: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: 'https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%202%2Fdapps.mp4?alt=media&token=be61b853-26f2-4b13-8c09-3e75abf0b8ab'}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be quickly touching base on DAPPS.`}
			</Text><></>
			<Text style={styles.para}>
			{`Let’s get started.`}
			</Text><></>
			...
			<Text style={styles.para}>
			{`DAPP stands for  Decentralized Application; an application that runs on a blockchain or P2P network of computers. `}
			</Text><></>
			<Text style={styles.para}>
			{`We are looking forward to an app being created that will take over the ecosystem; similar to how Facebook was created on the internet and produced a lot of traffic on the internet.`}
			</Text><></>
			<Text style={styles.para}>
			{`Majority of individuals are in cryptocurrency to make money, but if we could extend that into a real world application where blockchain technology is useful, besides financially, then there will be a mass adoption.`}
			</Text><></>
			<Text style={styles.para}>
			{`Looking around on DAPP websites, I usually use “State of the DApps” and “ DAPP Radar,” you will find many programs or games on this platform. `}
			</Text><></>
			<Text style={styles.para}>
			{`You may see gambling apps, I do not encourage gambling, on any platform, pr trying to make money from these apps. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Ultimately, we are keeping a look out for an app that is going to take over and change the ecosystem. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
