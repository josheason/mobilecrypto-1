...
export default class eth1016 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w2_6: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: 'https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%202%2Fethereum101.mp4?alt=media&token=c363f91c-eb58-46e3-aff2-91ca583c56ee'}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`At this point, you should understand what a Bitcoin is and how the ecosystem of blockchain technology works.`}
			</Text><></>
			<Text style={styles.para}>
			{`With that said, we are ready to jump into the basics of Ethereum. `}
			</Text><></>
			<Text style={styles.para}>
			{`Ethereum is a blockchain-based distributed computing platform.`}
			</Text><></>
			<Text style={styles.para}>
			{`The purpose of ethereum is so that blockchains can get decentralized technology on any application, besides money.  `}
			</Text><></>	
			<Text style={styles.para}>
			{`It is essentially the operating system. It can act as a form of money; however, it is a developer tool, so that developers can make other cryptocurrencies, like chainlink.`}
			</Text><></>	
			<Text style={styles.para}>
			{`ERC 20 tokens are apps on the blockchain, these create smart contracts. `}
			</Text><></>	
			<Text style={styles.bolPara}>
			{`How smart contracts work`}
			</Text><></>	
			<Text style={styles.para}>
			{`They are like any other contract, an agreement between two entities. `}
			</Text><></>	
			<Text style={styles.para}>
			{`It is automatically executed through the blockchain, without having to deal with third-parties. `}
			</Text><></>	
			<Text style={styles.para}>
			{`Essentially, creators upload content → users access content.`}
			</Text><></>	
			<Text style={styles.para}>
			{`In short, Ethereum is a bitcoin; but, instead of dealing specifically with money, they are trying to make a world operating system. By doing so, individuals will be able to use the same technology present in Bitcoin and put it to any use in the world. `}
			</Text><></>	
			<Text style={styles.para}>
			{`Now that we covered the basics, you will be prepared for the next video, Ethereum 102. `}
			</Text><></>	
			<Text style={styles.para}>
