...
export default class frontend1 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w10_1: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%2010%2FFrontEndBackEndMoney.mp4?alt=media&token=d1b90607-468c-4c59-a27e-fba0a961a7e5`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.bitlogreg5()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.zoom2()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about how to make money on the front end and back end.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`Local Bitcoin: 40% of 1% `}
			</Text><></>
			<Text style={styles.para}>
			{`Trading View: 30% of $30 `}
			</Text><></>
			<Text style={styles.para}>
			{`Fundamental Secrets LITE:$30 `}
			</Text><></>
			<Text style={styles.para}>
			{`Fundamental Secrets: $600 `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`It may not seem like alot, but it adds up over time. The more you refer, share links, or sell products, you will see the money.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Go ahead and make all of these accounts, have your affiliate links saved on your phone and computer in an easily accessible location so you can share it.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
