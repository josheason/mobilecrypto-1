...
export default class bigprom6 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w10_6: true})
        		})
        		batch.commit().then(() => {
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.caltutsales7()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`I recommend watching this video in order to see the examples I am discussing.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The hook is everything, for example, on Coinbase earn, a hook title would be “By the end of the call you will get $150 dollars” or “How to make $150 in ten minutes” `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`People want a reward with no effort, you want to take advantage of this in your title. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`You can also do this with a picture, gather testimonials and make a collage of them.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Write your promise multiple times, reword it and include it throughout the description in different ways. You want repetition, that will make people believe your promise.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Use helpful pictures, graphics, testimonials, a promise, and a positive description of what you are selling. You want to encourage consumers, not scare them. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`You will want to make a solid, thorough post because you’ll be copy and pasting this to post multiple times. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
