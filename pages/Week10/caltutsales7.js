...
export default class caltutsales7 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w10_7: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%2010%2FCalendlyTutorialForSales.mp4?alt=media&token=f3abf9e0-fd06-4f69-bb5b-b4824ad3edce`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.StartHere()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be doing a quick tutorial of Calendly, an app that schedules appointments, meetings and events.  `}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			<Text style={styles.para}>
			{`This video is tutorial style, so turn your attention to the screen (0:31) `}
			</Text><></>
			<Text style={styles.para}>
			{`Link Below:`}
			</Text><></>	
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Calendly: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://calendly.com/`)}>{`https://calendly.com/`}</Text><></>
			</Text><></>
