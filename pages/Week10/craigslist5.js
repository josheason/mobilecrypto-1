...
export default class craigslist5 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w10_5: true})
        		})
        		batch.commit().then(() => {
	...
	return(
				...
				{'Craigslist For Generating Leads'}
				</Text>
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%2010%2FCraigslistForGeneratingLeads.mp4?alt=media&token=a6c88886-b2f3-499a-886d-ed7e88113ba7`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.bigprom6()}>
		...
		>{
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Craigslist: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://www.craigslist.org/about/sites`)}>{`https://www.craigslist.org/about/sites`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
