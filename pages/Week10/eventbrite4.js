...
export default class eventbrite4 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w10_4: true})
        		})
        		batch.commit().then(() => {
	...
	return(
				...
				{'EventBrite For Lead Generation'}
				</Text>
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%2010%2FEventBriteForLeadGeneration.mp4?alt=media&token=1fb60d1f-384b-411d-8d0f-e859e67bdbc6`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.canva3()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.craigslist5()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about another platform called Eventbrite.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Go to Google Trends and look up where Bitcoin is being searched. Here we can see it is being searched in Las Vegas, Nevada.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Go on Eventbrite and you can host an online event, where it can be accessed from anyone.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`You can sell things to them by using your links and codes.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`It is a very good  tactic to make money.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Eventbrite: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://www.eventbrite.com/`)}>{`https://www.eventbrite.com/`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Google Trends: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://trends.google.com/trends/?geo=US`)}>{`https://trends.google.com/trends/?geo=US`}</Text><></>
			</Text><></>
