...
export default class zoom2 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w10_2: true})
        		})
        		batch.commit().then(() => {
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about zoom.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Turn your attention to the scream (0:17) to see how to open and navigate through zoom.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Zoom: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://zoom.us/`)}>{`https://zoom.us/`}</Text><></>
			</Text><></>
