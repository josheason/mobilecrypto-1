...
export default class partnerships6 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w5_6: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%205%2FPartnershipsUSE.mp4?alt=media&token=fc7bf02f-2ebb-4cb9-b9ea-07657880878c`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.corpgov7()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about partners.  `}
			</Text><></>
			<Text style={styles.para}>
			{`There are primarily 3 reasons why partners are important `}
			</Text><></>
			<Text style={styles.para}>
			{`1. Perception `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`Google Cloud partnered with Chainlink and it resulted in a huge following for Chainlink. `}
			</Text><></>
			<Text style={styles.para}>
			{`Similarly, JPMorgan partnered with Ethereum Alliance and Ethereum’s following experienced a  massive jump.  `}
			</Text><></>
			<Text style={styles.italPara}>
			{`Partnerships can play a big role. `}
			</Text><></>
			<Text style={styles.para}>
			{`When looking at cryptocurrency, you want to look at the partnerships they already have; however, you should keep watching for future partnerships. Doing this will allow you to invest more money down the road.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Now, let’s learn how to weed out fake partners.  `}
			</Text><></>
			<Text style={styles.para}>
			{`An important thing to look at is if the partners are speaking about the partnership as well; for example, Google posted about their partnership with Chainlink on their own blog. This proves the legitimacy of the partnership.  `}
			</Text><></>
			<Text style={styles.italPara}>
			{`Make sure it is publicly announced.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Doing research, you will easily find articles; but, you must dive deep to determine if the information is real.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
