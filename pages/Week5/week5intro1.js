...
export default class week5intro1 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w5_1: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%205%2FWeek5intro.mp4?alt=media&token=5e4273e2-3c91-44b3-a6c7-d8ef8ce69ec3`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.webedu8()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Now, you should understand the marketing strategies of companies.  `}
			</Text><></>
			<Text style={styles.para}>
			{`If you didn’t understand the last module, go back and redo it. It was very vital in building a foundation of knowledge for this section.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`This week we will talk about teams and leadership behind business.  `}
			</Text><></>
			<Text style={styles.para}>
			{`We are looking for high quality leaders; these are the people who decide what happens with your money. Good leaders will push the company into success. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Think of yourself as an investigator, you need to dive into the lives of these leaders to find out what makes them good or bad.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Again, make sure you are practicing good habits. Be sure to take notes and do research.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
