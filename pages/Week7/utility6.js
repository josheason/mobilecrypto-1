...
export default class utility6 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w7_6: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%207%2FUtilityUSE.mp4?alt=media&token=43e1b57c-1703-454d-a242-7e42589ed463`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about utility; what the actual use case for your product is.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Keep in mind the Blockchain is a ledger, not everything in the world will need this.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Blockchain fulfills censorship resistance, the reason why it is so useful.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Here are a couple questions to keep in mind when looking at a product: `}
			</Text><></>
			<Text style={styles.para}>
			...
			<Text style={styles.para}>
			{`3. Is it a “hair on fire” problem? (For instance: If your hair's on fire, you will buy a bucket of water for any amount, but you will not buy toothpaste. Essentially, is the product really needed? ) `}
			</Text><></>
			<Text style={styles.para}>
			{`4. Does this business model require censorship resistance?  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Ask yourself all of these questions and if it hits all of the criteria, then it is a great utility project. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
