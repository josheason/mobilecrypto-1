...
export default class ecopre5 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w7_5: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%207%2FEcosystemPredictions.mp4?alt=media&token=658e838f-3f3a-476a-a17e-68dd6b80c133`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.indfun4()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.utility6()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be doing some calculations on how much money there is in the world. In addition, we will be analyzing the ecosystem, diving deep into your coin, making comparisons and predictions.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`I predict Bitcoin will reach 10 trillion dollars; let’s compare it to the Gold market cap. `}
			</Text><></>
			<Text style={styles.para}>
			{`The Gold market cap is 9 trillion dollars. Therefore, I don’t think my prediction is unrealistic.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The total market cap of cryptocurrency is currently 254 billion dollars.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Cryptocurrency only has .28% of the world's money.  `}
			</Text><></>
			<Text style={styles.para}>
			{`All we need is 11% of the world's broad money to reach a 10 trillion dollar evaluation.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Pay attention to the circulating supply of the coins, you can do math to predict growth. `}
			</Text><></>
			<Text style={styles.para}>
			{`Compare the circulating supplies on different coins you’re interested in.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The market cap tells you what the cryptocurrency is worth, but it does not tell you how much you will make in percentage gain. `}
			</Text><></>
			<Text style={styles.para}>
			{`The smaller circulation supply, the more you make.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`Coin A has a bigger market cap and bigger circulating supply. `}
			</Text><></>
			<Text style={styles.para}>
			{`Coin B has a  smaller market cap and a  smaller circulating supply. `}
			</Text><></>
			<Text style={styles.para}>
			{`On a rating from 0-10: Coin A would score: 5/10 and Coin B would score: 10/10. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The key is to find which coin has the most potential to grow, among other fundamentals like a good team. `}
			</Text><></>
			<Text style={styles.para}>
			{`This all plays into the conclusion you must make when picking a coin.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Turn your attention to the screen for a demonstration on how to calculate (16:33) `}
			</Text><></>
			<Text style={styles.para}>
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`All Of The Worlds Money: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://www.visualcapitalist.com/worlds-money-markets-one-visualization-2017/`)}>{`https://www.visualcapitalist.com/worlds-money-markets-one-visualization-2017/`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`The World Factbook: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://www.cia.gov/library/publications/the-world-factbook/rankorder/2215rank.html`)}>{`https://www.cia.gov/library/publications/the-world-factbook/rankorder/2215rank.html`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Percentage Calculator: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://percentagecalculator.net/`)}>{`https://percentagecalculator.net/`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Coinmarketcap: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://coinmarketcap.com/`)}>{`https://coinmarketcap.com/`}</Text><></>
			</Text><></>
