...
export default class deskorg3 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3RF_3: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20RF%2FDesktopOrganization.mp4?alt=media&token=6f96dc1c-8cb2-49fe-ac51-45bb4adc702c`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.researchlist2()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today I will show you how to organize your research.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			<Text style={styles.para}>
			{`Hopefully, you did the research list we covered in the last video.`}
			</Text><></>	
			<Text style={styles.para}>
			{`The purpose of doing this organization is because you are going to want to save some of the information you find during your research.`}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			<Text style={styles.para}>
			{`You can do this by making a folder, let’s call it “Fundamental Secrets Research.”`}
			</Text><></>	
			<Text style={styles.para}>
			{`Then, you will split separate files according to the different ecosystems. `}
			</Text><></>	
			<Text style={styles.para}>
			{`If you take notes on my videos, you can also save them in the folder. `}
			</Text><></>	
			<Text style={styles.para}>
			{`Using this organization method will allow you to go back and access your notes and  research much easier. `}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>	
