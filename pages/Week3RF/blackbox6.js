...
export default class blackbox6 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3RF_6: true})
        		})
        		batch.commit().then(() => {
	...
	return(
				...
				{'The Black Box and Perfectionism'}
				</Text>
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20RF%2FTheBlackBoxandPerfectionism.mp4?alt=media&token=6f860235-3132-4369-8b91-4bf84de6b8ff`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about perfectionism and I will explain the concept of the black box. `}
			</Text><></>
			<Text style={styles.para}>
			{`I struggled with perfectionism when I was first diving into cryptocurrency; I am sure many others will face struggles with this as well.`}
			</Text><></>
			<Text style={styles.para}>
			{`The black box was designed to explain how to figure out hard problems that no one has the answer to.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The goal is to repetitively test inputs, to find a positive output.`}
			</Text><></>
			<Text style={styles.para}>
			{`There is no way to accurately, 100%, predict the value of the market; However, what you can do is analyze inputs and know the history of outputs, so you will be able to make knowledgeable predictions of the future. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.italPara}>
			{`Do not let perfectionism hinder your success. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
