...
export default class whitepaper1015 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3RF_5: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20RF%2FWhitepaper101.mp4?alt=media&token=90a5ef04-9316-48cb-b8fd-890b67973393`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.avoidcred4()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.blackbox6()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about the whitepaper, which is essentially a business plan.`}
			</Text><></>
			<Text style={styles.para}>
			{`The whitepaper is one of the most important things about cryptocurrency.`}
			</Text><></>
			<Text style={styles.para}>
			{`It is the best way to research a company. However, you must compare whitepapers against each other, to differentiate what material is good. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The first thing you must do is read the Bitcoin whitepaper, this is very important.`}
			</Text><></>
			<Text style={styles.para}>
			{`Second, you must read the Ethereum whitepaper. `}
			</Text><></>
			<Text style={styles.para}>
			{`Reading these two whitepapers will give you a wealth of knowledge;  they are very well thought out, basically the premise of the whole business. `}
			</Text><></>
			<Text style={styles.para}>
			{`Read them with intent, take notes, and try to understand it. `}
			</Text><></>
			<Text style={styles.italPara}>
			{`I strongly urge you to do this. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`For this video, we will look at the whitepaper for Energi. `}
			</Text><></>
			<Text style={styles.para}>
			{`To find the whitepaper for a coin, simply type it into google. For example, “Energi Whitepaper.”`}
			</Text><></>
			<Text style={styles.para}>
			{`If it is lengthy, that is a sign of a good whitepaper. If you can’t get through it, at least, read the beginning and the end. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`When trying to gather information, Youtube videos can be very helpful, but ultimately it is someone else’s opinion. `}
			</Text><></>
			<Text style={styles.para}>
			{`The whitepaper gives you the opportunity to read it yourself and form your own opinion. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
