...
export default class avoidcred4 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3RF_4: true})
        		})
        		batch.commit().then(() => {
	...
	return(
				...
				{'Avoid FUD With Checking Credibility'}
				</Text>
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20RF%2FAvoidFUDWithCheckingCredibility.mp4?alt=media&token=f300ac07-55f8-44b0-98a2-8319919c66ba`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.deskorg3()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.whitepaper1015()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`In cryptocurrency, you will hear the term “FUD” very often. FUD stands for: Fear Uncertainty and Doubt. `}
			</Text><></>	
			<Text style={styles.para}>
			{`As you get more involved in cryptocurrency, you will find yourself reading more articles. However, it is important that you verify the information provided is not false.`}
			</Text><></>		
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			<Text style={styles.para}>
			{`There was an instance where someone mimicked a crypto website and released an article and video urginging people to buy a coin; this resulted in chaos among many. `}
			</Text><></>		
			<Text style={styles.italPara}>
			{`There are people out there who want to trick you.`}
			</Text><></>	
			<Text style={styles.para}>
			{`On the other hand, there are a bit more reliable sources that give somewhat factual articles, but they are pushed so hard to release content that it ends up being very exaggerated. `}
			</Text><></>		
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			<Text style={styles.para}>
			{`Therefore, today we will be going over how to avoid this by checking credibility. `}
			</Text><></>		
			<Text style={styles.para}>
			{`Follow along with me, as we go through some articles; turn your attention to the screen (4:40)`}
			</Text><></>		
