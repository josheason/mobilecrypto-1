...
export default class stockflowx3 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w9_3: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%209%2FStockToFlowXS2FX.mp4?alt=media&token=9b9ed2e7-b309-4a46-bf83-b241c46132a90`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.willywoo4()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about stock to flow X, this is different from stock to flow ratio; if you have not watched that video, go back and watch it before continuing.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`I will be going through an article that explains this really well on medium.om by PlanB. You will  definitely want to follow him, he is really good with data science.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Phase Transitions are an important perspective in understanding S2FX models. During a phase transition, properties change. Examples are: Water, USD, BTC. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Money is essentially energy being transferred, just like Bitcoin.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Going into the phases of Bitcoin, we can say that phase one was “proof of concept” so people were very curious about it and it proved itself as valuable. The second phase was payments. The third transition was “E-Gold” after the 1st halving. Then, the fourth phase was it becoming a  “financial asset” after the 2nd halving.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`They use these 4 clusters to predict the future.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`I will leave the link to this article in the description, read it for yourself, more than once if you have to.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			onPress={() => Linking.openURL(`https://medium.com/@100trillionUSD/bitcoin-stock-to-flow-cross-asset-model-50d260feed12`)}>{`https://medium.com/@100trillionUSD/bitcoin-stock-to-flow-cross-asset-model-50d260feed12`}</Text><></>
			</Text><></>
