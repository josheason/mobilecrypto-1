...
export default class stock2flow2 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w9_2: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%209%2FStockToFlowModel.mp4?alt=media&token=0fb2b56a-aab1-4437-8ac7-51e85be36c43`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.stockflowx3()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about a popular data science chart: stock to flow ratio. I will break it down and go into all of the details.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Essentially, It treats Bitcoin as comparable to commodities like gold and silver.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`This was made for valuable commodities that have a certain amount of flow versus the stock.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`As we know, Bitcoin is scarce. If you don’t know what I’m talking about, refer back to week 2, where I discussed Bitcoin 101 and 102.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Now turn your attention to the screen (1:32)  to learn how to read this chart. `}
			</Text><></>
