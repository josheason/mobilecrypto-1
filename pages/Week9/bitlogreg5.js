...
export default class bitlogreg5 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w9_5: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%209%2FBitcoinLogarithmicRegression.mp4?alt=media&token=24126b9f-7f98-4efa-aa89-dd00a8942154`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.frontend1()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about Bitcoin logarithmic regression. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`I want to add that I do not make any of these charts; I want to give Benjamin Cohen credit, I use his charts very often in my course.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`He made a “Logarithmic Regression of Bitcoin Prices” chart that I want to go through with you.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Turn your attention to the screen  (0:27) to analyze this chart with me. `}
			</Text><></>
