...
export default class details4 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_4: true})
        		})
        		batch.commit().then(() => {
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.coinbase3()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.coinbasepro5()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today I will be doing very basic math calculations, that I believe everyone should know before getting into cryptocurrency.`}
			</Text><></>
			<Text style={styles.para}>
			{`You will be using it to calculate different variables to see how much money you have.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The first one will be fairly easy, we are trying to figure out how much your cryptocurrency is worth.`}
			</Text><></>
			<Text style={styles.para}>
			{`Let’s say, hypothetically, your wallet says you have .056 worth of bitcoin; to figure out how much this is in US Dollars, multiply that number (.056) by the current price of Bitcoin, which is $9,918. You will end up with $555.41 in US Dollars.`}
			</Text><></>
			<Text style={styles.para}>
			{`In contrast, if you have the amount in US Dollars and you want to know what it will be worth in Bitcoin, do the opposite. `}
			</Text><></>
			<Text style={styles.para}>
			{`Take your number, let's say $500, and divide it by the current price of Bitcoin (9,918). You will end up with 0.050 worth of Bitcoin. `}
			</Text><></>
			<Text style={styles.para}>
			{`Now, you're not always going to be calculating in USD. You will be calculating between other cryptocurrencies in Bitcoin terms. `}
			</Text><></>
			<Text style={styles.italPara}>
			{`The goal here is to increase your Bitcoin position, not your cash position. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The second calculation technique is to go on trading view (tradingview.com)`}
			</Text><></>
			<Text style={styles.para}>
			{`Using this chart as a tool, click where you would like to start and end, then you will be able to view the percentage you've lost/made. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`For the third calculation technique, let’s say hypothetically you're on the dashboard, you would like to purchase cryptocurrency and allocate your portfolio similar to mine. `}
			</Text><></>
			<Text style={styles.para}>
			{`If you want to put 2.7% of your money into this coin - to follow my profolio right all you have to do is take $1,000 and multiply it by 0.027; this will result in the amount of 27.`}
			</Text><></>
			<Text style={styles.para}>
			{`This means that if you had $1,000 and you wanted to copy my portfolio, you would be putting in $27.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Hopefully, this makes sense to you all; these techniques are just a few details I wanted you to know. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
