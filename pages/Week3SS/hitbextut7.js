...
export default class hitbextut7 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_7: true})
        		})
        		batch.commit().then(() => {
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.deskwall8()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about Hit BTC ( I will leave the website link below). `}
			</Text><></>		
			<Text style={styles.para}>
			{`I will be going through the various parts of the exchange.`}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			<Text style={styles.para}>
			{`I have already reviewed Coinbase and Binance exchange platforms, they are all not very far off from each other.`}
			</Text><></>		
			<Text style={styles.para}>
			{`The similarities put the videos at risk of being very repetitive; however, if you all wish for me to create more, I do not mind.`}
			</Text><></>		
			<Text style={styles.para}>
			{`It simply requires perusing the website to really get the hang of each one. `}
			</Text><></>		
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			...
			<Text style={styles.para}>
			{`Now, turn your attention to the screen (1:05)`}
			</Text><></>		
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`HitBTC: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://hitbtc.com/`)}>{`https://hitbtc.com/`}</Text><></>
			</Text><></>
