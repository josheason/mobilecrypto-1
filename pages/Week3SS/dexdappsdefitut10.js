...
export default class dexdappsdefitut10 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_10: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20SS%2FDEXDAPPSDEFI%20Tutorial.mp4?alt=media&token=80be6e67-7c88-4a00-a3c6-07a4bbc9bdfc`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about DEX, DAPPS, and DEFI.`}
			</Text><></>
			<Text style={styles.para}>
			{`You will be getting a 3-in-1 tutorial.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			<Text style={styles.para}>
			{`This is not pre-setup; I will be doing it in real time, so you can see how I solve problems when issues arise. `}
			</Text><></>	
			<Text style={styles.para}>
			{`The three websites I usually look at are: Dex Watch, Defi Pulse, and Dapp Radar (links below).`}
			</Text><></>	
			<Text style={styles.para}>
			{`Hopefully, you already have the Metamask wallet downloaded.`}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			<Text style={styles.para}>
			{`This video will be tutorial style.`}
			</Text><></>	
			<Text style={styles.para}>
			{`Now, turn your attention to the screen (1:10).  `}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Kyberswap: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://kyber.network/`)}>{`https://kyber.network/`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Dex Watch: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://dex.watch/`)}>{`https://dex.watch/`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Defi Pulse: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://defipulse.com/`)}>{`https://defipulse.com/`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`MakerDAO: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://makerdao.com/en/`)}>{`https://makerdao.com/en/`}</Text><></>
			</Text><></>
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Dapp Radar: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://dappradar.com/`)}>{`https://dappradar.com/`}</Text><></>
			</Text><></>
