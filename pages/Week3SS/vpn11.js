...
export default class vpn11 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_11: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20SS%2FVPNForToBypassAuthority.mp4?alt=media&token=066b68fe-8005-45dc-906c-afc740e43c14`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.dexdappsdefitut10()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.googleauth12()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about how to get past government restrictions.`}
			</Text><></>
			<Text style={styles.para}>
			{`I will show you how to use a Virtual Private Network, VPN, to bypass authority. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`All you have to do is download a free VPN service; I use Proton VPN (link below).`}
			</Text><></>
			<Text style={styles.para}>
			{`Connect to a country, let’s say Belgium.`}
			</Text><></>
			<Text style={styles.para}>
			{`Open the BitMex website; now you have access, there is no error code.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Ultimately, the purpose of knowing how to do this is if the US banned an exchange, you can use this method to gain access to it.`}
			</Text><></>
			<Text style={styles.para}>
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`VPN: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://protonvpn.com/`)}>{`https://protonvpn.com/`}</Text><></>
			</Text><></>
