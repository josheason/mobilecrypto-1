...
export default class googleauth12 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_12: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20SS%2FGoogleAuthenticator.mp4?alt=media&token=eabbe028-ec03-4358-8fbe-11f6ee376474`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today I will be teaching you how to set up 2-factor authentication with google.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			<Text style={styles.para}>
			{`On your computer, go on any exchange (I am using Hit BTC), go to account → security→ add new authenticator `}
			</Text><></>		
			<Text style={styles.para}>
			{`Download the Google Authenticator app on your mobile device. `}
			</Text><></>		
			<Text style={styles.para}>
			{`Scan the barcode that appears on your computer. `}
			</Text><></>		
			<Text style={styles.para}>
			{`Confirm by entering the code that appears on your mobile device. `}
			</Text><></>		
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			...
			<Text style={styles.para}>
			{`After entering the password, it will ask for authenticator code. `}
			</Text><></>		
			<Text style={styles.para}>
			{`Enter the code shown on the app and you should log in smoothly.`}
			</Text><></>		
			<Text style={styles.para}>
			{` `}
			</Text><></>		
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Google Authenticator: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=en_US`)}>{`https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=en_US`}</Text><></>
			</Text><></>
