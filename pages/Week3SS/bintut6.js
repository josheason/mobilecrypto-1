...
export default class bintut6 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_6: true})
        		})
        		batch.commit().then(() => {
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about Binance (I will leave the website link below).`}
			</Text><></>
			<Text style={styles.para}>
			{`Ultimately, we are using this platform as a way to access different cryptocurrencies that are not found on Coinbase.`}
			</Text><></>
			<Text style={styles.para}>
			{`We will not dive deep into Binance, but I will show you how to make money on it.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`I am open to reviewing any exchanging platform; if there is a newer or more popular platform you would like me to go over, reach out to me. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`Now, turn your attention to the screen (0.59).`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			...
			<Text style={styles.para}>
			<Text style = {styles.reg}>{`Binance: `}</Text><></>
			<Text style = {styles.regURL}
			onPress={() => Linking.openURL(`https://www.youtube.com/redirect?q=https%3A%2F%2Fwww.binance.com%2F%3Fref%3D11610125&v=XC0iXYK0FSg&redir_token=N6ml7ANkgZWbihtiKzj8c_4Lj0B8MTU4NzE0NDYwMkAxNTg3MDU4MjAy&event=video_description`)}>{`https://www.youtube.com/redirect?q=https%3A%2F%2Fwww.binance.com%2F%3Fref%3D11610125&v=XC0iXYK0FSg&redir_token=N6ml7ANkgZWbihtiKzj8c_4Lj0B8MTU4NzE0NDYwMkAxNTg3MDU4MjAy&event=video_description`}</Text><></>
			</Text><></>
