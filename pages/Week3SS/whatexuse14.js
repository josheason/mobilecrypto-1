...
export default class whatexuse14 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_14: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20SS%2FWhatExchangeShouldIUse.mp4?alt=media&token=9e9a20a3-a8cc-4671-9234-ee2a62364816`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.keepass13()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.ultimsec15()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today I will be answering a commonly asked question “What exchanges should I use?” `}
			</Text><></>
			<Text style={styles.para}>
			{`There is no one particular exchange, the best strategy is to use multiple exchanges. `}
			</Text><></>
			<Text style={styles.italPara}>
			{`It does not matter what exchange you use.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`A bank exchange turns your fiat currency into cryptocurrency. `}
			</Text><></>
			<Text style={styles.para}>
			{`The bank exchange I use is called Coinbase. `}
			</Text><></>
			<Text style={styles.para}>
			{`This is going to pull money from your debit or credit card and turn it into cryptocurrency. `}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Do not buy Allcoins directly; I recommend only buying big market cap coins, like bitcoin. This is because when you buy a coin you can not transfer it for 3-5  days, you do not want to lose money if the coin dumps. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`The same goes for cryptocurrency to cryptocurrency exchange, it is not relevant. `}
			</Text><></>
			<Text style={styles.para}>
			{`All you have to do to find out what exchange has your cryptocurrency is: go on coinmarketcap.com → choose a coin → go to market pairs. Here, you will find what exchange holds your coin of choice. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`You are never going to leave your money on these exchanges, so it does not matter which one you choose. `}
			</Text><></>	
			<Text style={styles.para}>
			{`You should always send your money to your desktop wallet or cold storage, as we discussed in a previous video.`}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
