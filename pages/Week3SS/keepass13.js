...
export default class keepass13 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w3SS_13: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%203%20SS%2FKeePass.mp4?alt=media&token=248d311f-a6a2-4c0b-b616-ad9b107516a9`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about a software that I use called Keypass, a software that stores all of your passwords.`}
			</Text><></>	
			<Text style={styles.para}>
			{`Let’s jump into it.`}
			</Text><></>	
			...
			<Text style={styles.para}>
			{`All you have to do is create a new database; a database is basically a file that holds all of your passwords.`}
			</Text><></>	
			<Text style={styles.para}>
			{`You have a master password, that protects the rest of your passwords.`}
			</Text><></>	
			<Text style={styles.para}>
			{`An “Emergency Sheet '' will be prompted on the screen, you do not have to print this, but keep in mind you are financially relying on these passwords; it could save you one day. `}
			</Text><></>	
			<Text style={styles.para}>
			{`Now, I will show you how to add new entries and generate a random password. `}
			</Text><></>	
			<Text style={styles.para}>
			{` `}
			</Text><></>	
			<Text style={styles.para}>
			{`Turn your attention to the screen (2:20)`}
			</Text><></>	
