...
export default class rewire8 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w1_8: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: 'https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%201%2Fthreewaysrewirebrain.mp4?alt=media&token=9c13724d-7896-4e2e-a78a-514bb66d8a40'}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			{`Welcome back!`}
			 ...
			 <Text style={styles.para}>
			{`Today, I will undertake a massive psychological change in the way you think.`}
			</Text><></>
			<Text style={styles.para}>
			{'Technophobia, the fear of change and advancement in technology, can be traced back to many years ago, when computers were on the rise. '}
			</Text><></>
			<Text style={styles.para}>
			{'People were afraid that computers could brainwash them or take over the world. '}
			</Text><></>
			<Text style={styles.para}>
			{`This can still be seen in some ways today. Many individuals are afraid of internet 3.0 decentralization; there is a lot of skepticism and doubt that surrounds it.`}
			</Text><></>
			<Text style={styles.para}>
			{`I am going to tackle this fear by giving you 3 steps on how to rewire your brain. `}
			</Text><></>
			<Text style={styles.bolPara}>
			{`Step 1: Your learning Method`}
			</Text><></>
			<Text style={styles.para}>
			{`Verbal | Physical | Logical`}
			</Text><></>	
			<Text style={styles.para}>
			{`Do not just watch the videos; do research, read the books that I am recommending, invest time and care into the course. `}
			</Text><></>
			<Text style={styles.para}>
			{`By doing this, you will ingrain it into your head.`}
			</Text><></>
			<Text style={styles.para}>
			{`You need to figure out your learning method.  `}
			</Text><></>
			<Text style={styles.para}>
			{`If you are a verbal person: you need to communicate with other people. `}
			</Text><></>
			<Text style={styles.para}>
			{`If you are a physical learner: you may need to watch a lot of videos.   `}
			</Text><></>
			<Text style={styles.para}>
			{`If you are a logical learner: you may spend more time in the technical analysis section or working with numbers.`}
			</Text><></>
			<Text style={styles.para}>
			{`Although you should touch base with each learning style, it is vital that you figure out which one closely relates to you in order to take full advantage of it. `}
			</Text><></>
			<Text style={styles.bolPara}>
			...
			<Text style={styles.para}>
			{`It is important to engage all of your senses into your learning process. `}
			</Text><></>
				
			<Text style = {styles.para}>
			<Text style = {styles.reg}>{`You need to `}</Text><></><Text style = {styles.ital}>{`listen `}</Text><></><Text style = {styles.reg}>{`to podcasts and audio books.`}</Text><></>	
			</Text><></>
				
			<Text style = {styles.para}>
			<Text style = {styles.reg}>{`You need to `}</Text><></><Text style = {styles.ital}>{`watch `}</Text><></><Text style = {styles.reg}>{`videos to see demonstrations and gain insight from others.`}</Text><></>	
			</Text><></>
			
			<Text style = {styles.para}>
			<Text style = {styles.reg}>{`You need to `}</Text><></><Text style = {styles.ital}>{`feel `}</Text><></><Text style = {styles.reg}>{` it by actually applying the concepts you’ve learned; go out and do it.`}</Text><></>	
			</Text><></>
			...
			<Text style={styles.bolPara}>
			{`This rule is the reason why I made the Facebook group and encourage you all to use it.  `}
			</Text><></>
			<Text style={styles.para}>
			{`In the group you will find others that are below you, this gives you an opportunity to teach. Teaching others can help strengthen your own understanding on a topic.`}
			</Text><></>
			<Text style={styles.para}>
			{`You will also find someone at the same level as you, this presents a challenge. Having someone to compete with can be a strong motivator. On the other hand, those on the same level as you can help you grow. You will have someone to figure it out with along the way. `}
			</Text><></>
			<Text style={styles.para}>
			{`Most importantly, you need someone above you to mentor you. It is nice to have an experienced individual by your side when you have questions or need advice. `}
			</Text><></>
			<Text style={styles.para}>
			{`Ultimately, it will benefit your progress and success in this course to create new, positive habits using these 3 steps.  `}
			</Text><></>
			<Text style={styles.para}>
