...
export default class googadwordcrypto4 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w4_4: true})
        		})
        		batch.commit().then(() => {
	...
	return(
				...
				{'Google Ad-words For Cryptocurrencies'}
				</Text>
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%204%2FGoogleAd-wordsForCryptocurrencies.mp4?alt=media&token=d276918c-d99b-4118-9dad-2f9befa752cd`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.scrapytdata5()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`In this video we will be looking into Google Ads to dissect search terms.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Go on Google and search anything, let’s say “pizza” `}
			</Text><></>
			<Text style={styles.para}>
			{`This search term will bring you to a specific page. Here, you will see at the very top there is an Ad; a company is paying for this top spot, in order to get more website traffic.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`If we were able to see the search term history for cryptocurrencies, it would give us plenty of insight on their value.  `}
			</Text><></>
			<Text style={styles.para}>
			{`To do this, you must first go on Google Ads and create a campaign; you will be prompted to enter credit card information (cancel it, so you do not get charged). `}
			</Text><></>
			<Text style={styles.italPara}>
			{`Note: You will not get access to insight without creating a campaign.  `}
			</Text><></>
			<Text style={styles.para}>
			{`When you are in go to: `}
			</Text><></>
			<Text style={styles.para}>
			{`Tools and settings → keyword planner `}
			</Text><></>
			<Text style={styles.para}>
			{`The keyword planner will show you various keywords, the average monthly searches, competition, and the highest and lowest bids.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Type in any coin; let’s use “bitcoin” `}
			</Text><></>
			<Text style={styles.para}>
			{`You can see how much the company is paying for an ad. This lets you know how well they are doing.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.italPara}>
			{`The trick is to analyze what you see and draw conclusions. `}
			</Text><></>
			<Text style={styles.para}>
			{`When researching a coin, it is helpful to  look them up on Google ads and dissect their average monthly searches and bids.  `}
			</Text><></>
			<Text style={styles.italPara}>
			{`This is a good way to weed out the bad coins. `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Ultimately, I want you all to have this in your tool belt. It is a nice trick to gain insight on a company.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
