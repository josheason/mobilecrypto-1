...
export default class webtraff7 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w4_7: true})
        		})
        		batch.commit().then(() => {
	...
	//		posterResizeMode={'contain'}
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be analyzing website traffic.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`There are some trackers that require payment; however I do not find it necessary to buy them. `}
			</Text><></>
			<Text style={styles.para}>
			{`Here are three (free) tools you can use to track: SpyFu, SimilarWeb, and Semrush.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Follow along with me, as I show you how to track website traffic (0:59) `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			...
			<Text style={styles.para}>
			{`1. Pay attention to the total visits of a website. `}
			</Text><></>
			<Text style={styles.para}>
			{`2. Compare analytics  between all three trackers; you will notice they have different numbers. `}
			</Text><></>
			<Text style={styles.para}>
			{`3. There is no way to perfectly pinpoint these numbers, we are just analyzing variables and looking at all inputs and outputs to draw a conclusion.  `}
			</Text><></>
