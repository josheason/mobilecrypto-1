...
export default class scrapytdata5 extends Component{
   		...
   		watchedRef.where('id', '==', user.uid)
     		 ...
     		 .get().then(response => {
        		...
        		response.docs.forEach((doc) => {
            		...
            		batch.update(docRef, {watched: increment , w4_5: true})
        		})
        		batch.commit().then(() => {
	...
	return(
			...
			source={{ uri: `https://firebasestorage.googleapis.com/v0/b/mobilecrypto-b1543.appspot.com/o/Week%204%2FScrapingYouTubeforDATA.mp4?alt=media&token=12a38492-5462-4943-b00f-7930940e972f`}}
			rate={1.0}
			volume={1.0}
			isMuted={false}
			resizeMode="cover"
	...
	//		posterResizeMode={'contain'}
			...
			/><></>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.googadwordcrypto4()}>
				...
				<TouchableOpacity activeOpacity={0.3} underlayColor="#6200ee" onPress={() => Actions.webconvdes6()}>
		...
		>{
			...
			<Text style={styles.para}>
			{`Today we will be talking about using YouTube as a search platform.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Yes, Youtube is a place to watch videos; but it is a search algorithm  first.  `}
			</Text><></>
			<Text style={styles.para}>
			{`Individuals go on Youtube to learn new information; this is why how-to, DIY, and informative videos are the most popular.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`With that being said, it is important to weed out who is providing accurate, credible videos.  `}
			</Text><></>
			<Text style={styles.para}>
			{`To do this, download VidIQ, a chrome extension.  `}
			</Text><></>
			<Text style={styles.para}>
			{`VidIQ will allow you to see analytics, such as: highest views, average views, subscribers, and search traffic on each keyword.  `}
			</Text><></>
			<Text style={styles.para}>
			{` `}
			</Text><></>
			<Text style={styles.para}>
			{`Follow along with me, as I analyze videos and scrape YouTube for data (2:10) `}
			</Text><></>
